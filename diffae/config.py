...
data_paths = {
    ...
    'afhq256-cat':
    os.path.expanduser('datasets/afhq-cat/train'),
    'afhq256-dog':
    os.path.expanduser('datasets/afhq-dog/train'),
    'church256':
    os.path.expanduser('datasets/church/train'),
}
    ...
    def make_dataset(self, path=None, **kwargs):
        if self.data_name == 'ffhqlmdb256':
            ...
            return CelebAlmdb(path=path or self.data_path,
                              ...
                              crop_d2c=True,
                              **kwargs)
        elif self.data_name == 'afhq256-cat':
            return AFHQlmdb(path=path or self.data_path,
                            image_size=self.img_size,
                            **kwargs)
        elif self.data_name == 'afhq256-dog':
            return AFHQlmdb(path=path or self.data_path,
                            image_size=self.img_size,
                            **kwargs)
        elif self.data_name == 'church256':
            return Horse_lmdb(path=path or self.data_path,
                              image_size=self.img_size,
                              **kwargs)
        else:
